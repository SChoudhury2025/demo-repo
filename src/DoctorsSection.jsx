...
const DoctorsSection = () => {
  ...
  // Chunk doctors into groups of 4 per slide
  const chunkedDoctors = [];
  for (let i = 0; i < doctors.length; i += 4) {
    chunkedDoctors.push(doctors.slice(i, i + 4));
  }

  // Auto-slide effect: change slide every 5 seconds
  useEffect(() => {
    if (chunkedDoctors.length === 0) return;

    const interval = setInterval(() => {
      setCurrentSlide(prev => (prev === chunkedDoctors.length - 1 ? 0 : prev + 1));
    }, 5000);

    return () => clearInterval(interval); // cleanup on unmount or chunk change
  }, [chunkedDoctors.length]);

  const nextSlide = () => {
    setCurrentSlide(prev => Math.min(prev + 1, chunkedDoctors.length - 1));
  };
        ...
        <div className="relative">
          <div className="overflow-hidden">
            ...
            >
              ...
              ) : chunkedDoctors.length > 0 ? (
                chunkedDoctors.map((group, slideIndex) => (
                  <div key={slideIndex} className="flex-shrink-0 flex w-full">
                    {group.map((doctor, index) => (
          ...
          {/* Navigation Arrows */}
          <button
            className={`absolute left-0 top-1/2 -translate-y-1/2 bg-white shadow-md rounded-full p-3 focus:outline-none z-10 ${
          ...
          <button
            className={`absolute right-0 top-1/2 -translate-y-1/2 bg-white shadow-md rounded-full p-3 focus:outline-none z-10 ${
              currentSlide >= chunkedDoctors.length - 1 ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-100'
            }`}
            onClick={nextSlide}
            disabled={currentSlide >= chunkedDoctors.length - 1}
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-gray-800" fill="none" viewBox="0 0 24 24" stroke="currentColor">
